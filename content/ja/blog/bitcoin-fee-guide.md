...
[UTXOモデルを理解する](/ja/blog/utxo-model/)で確認したように、入力数が多いほど手数料が急激に増加する。
...
[txid.uk](https://txid.uk/) のメインページで現在の推奨手数料率をリアルタイムで確認できる。
...
[stats.txid.uk](https://stats.txid.uk/)ではブロック平均手数料、ブロックあたりのトランザクション数などオンチェーン統計を時系列で確認でき、手数料のトレンド把握に有用だ。
...
お釣り出力のないトランザクションを**最適トランザクション（optimal transaction）**という。複数の支払先を1つのトランザクションにまとめる**バッチ処理（batch processing）**も効率的だ。取引所や企業で多く活用されている。
...
- **[txid.uk](https://txid.uk/) 手数料計算機：** メインページで現在の手数料率と予想コストをリアルタイムで確認
- **[tools.txid.uk](https://tools.txid.uk/) 単位変換ツール：** sat ↔ BTC ↔ JPY/USD変換で手数料の実際のコストを把握
- **[tx.txid.uk](https://tx.txid.uk/) TXデコーダー：** Rawトランザクションをデコードして実際のサイズ（vbytes）と手数料を確認
- **Sparrow Wallet：** ウォレット内でトランザクションサイズと手数料をリアルタイムで表示
- **Bitcoin Core：** `estimatesmartfee` RPCコマンドで目標確認ブロック数に合った手数料率を照会可能

## まとめ

ビットコインの手数料は送金金額ではなくトランザクションのデータサイズに比例する。SegWitアドレスの使用、UTXO数の最小化、混雑時間帯の回避で手数料を効果的に削減できる。sat/vbyteの単位を理解し、現在のメンプール状態を確認する習慣がビットコインを賢く使う上で重要だ。
