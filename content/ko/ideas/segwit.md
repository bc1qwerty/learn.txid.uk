...
graph LR
    subgraph "기존 트랜잭션"
        A1["버전"] --- A2["입력<br/>(스크립트+서명)"] --- A3["출력"] --- A4["록타임"]
    end
...
**빅 블록 진영**은 블록 크기를 2MB, 8MB, 혹은 그 이상으로 직접 증가시키는 하드 포크를 주장했습니다. 더 큰 블록이 더 많은 트랜잭션을 처리할 수 있으므로 즉각적인 확장이 가능하다는 논리였습니다. 이 진영에는 다수의 대형 채굴 풀과 일부 기업이 포함되어 있었습니다.

**스몰 블록 진영**은 블록 크기 증가가 노드 운영 비용을 높여 탈중앙화를 약화시킨다고 우려했습니다. 대신 세그윗과 같은 소프트 포크를 통한 효율적 확장, 그리고 라이트닝 네트워크 같은 2계층 솔루션을 통한 장기적 확장을 지지했습니다. 핵심 개발자 대부분과 다수의 노드 운영자가 이 입장이었습니다.
...
- [라이트닝 네트워크](/ko/ideas/lightning-network/) — 세그윗 위에 구축된 비트코인 2계층 결제 네트워크
- [노드](/ko/ideas/node/) — UASF를 통해 세그윗 활성화를 주도한 비트코인 네트워크의 독립적 검증 소프트웨어
- [머클 트리](/ko/ideas/merkle-tree/) — 세그윗이 증인 데이터를 위한 별도의 머클 구조를 도입
- [멤풀](/ko/ideas/mempool/) — 세그윗의 가중치 시스템이 수수료 구조에 영향
