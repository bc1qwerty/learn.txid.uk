...
비트코인 노드들은 중앙 서버 없이 P2P 가십 프로토콜을 통해 통신합니다. 새로운 트랜잭션이나 블록이 발견되면, 해당 정보를 먼저 `inv` 메시지로 연결된 피어에게 알리고, 피어가 `getdata`로 요청하면 실제 데이터를 전송합니다. 이 방식으로 정보가 네트워크 전체에 수 초 내에 확산됩니다.
...
- [작업증명](/ko/ideas/proof-of-work/) — 블록 생성을 위한 에너지 기반 합의 메커니즘
- [난이도 조정](/ko/ideas/difficulty-adjustment/) — 블록 생성 속도를 일정하게 유지하는 알고리즘
- [세그윗](/ko/ideas/segwit/) — 노드의 UASF로 활성화된 비트코인 프로토콜 업그레이드
- [멤풀](/ko/ideas/mempool/) — 노드가 유지하는 미확인 트랜잭션 대기 공간
