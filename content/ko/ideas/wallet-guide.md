...
수천만 원 이상의 비트코인을 보관한다면, 단일 지갑에 의존하는 것은 위험하다. [멀티시그](/ideas/multisig/)를 설정하여 여러 개의 키 중 일정 수(예: 3개 중 2개)의 서명이 있어야 거래를 승인하도록 한다. 단일 지점 실패(기기 분실, 도난)를 근본적으로 방지한다.

### 시나리오 4: 일상 결제 사용자

**추천**: 라이트닝 지원 모바일 지갑 (Phoenix) + 하드웨어 지갑 병행

비트코인으로 커피를 사거나 소액을 보내려면 [라이트닝 네트워크](/ideas/lightning-network/)가 필수다. Phoenix 같은 라이트닝 지원 지갑에 소액을 넣어 결제용으로 쓰고, 나머지 대부분은 하드웨어 지갑에 보관한다. 지갑 속 현금과 은행 계좌를 분리하는 것과 같은 원리다.
...
- [멀티시그](/ideas/multisig/) — 다중 서명으로 단일 지점 실패를 방지하는 보안 메커니즘
- [라이트닝 네트워크](/ideas/lightning-network/) — 비트코인의 일상 결제를 가능하게 하는 2계층 솔루션
- [노드](/ideas/node/) — 자신의 거래를 직접 검증하는 비트코인 풀 노드
