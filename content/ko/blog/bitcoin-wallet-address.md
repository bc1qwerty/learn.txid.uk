---
title: "비트코인 지갑 주소란 무엇인가"
date: 2026-03-04T14:00:00+09:00
description: "비트코인 지갑 주소의 구조와 종류, 생성 원리, 올바른 사용법까지. P2PKH, P2SH, 네이티브 SegWit(Bech32) 주소를 초급 독자 눈높이에서 설명한다."
tags: ["비트코인"]
levels: "입문"
weight: 20
---
...
공개키에 SHA-256과 RIPEMD-160 해시 함수를 순차 적용하고, Base58Check 인코딩을 거쳐 최종 주소가 생성된다. 이 과정에서도 역산은 불가능하다.
...
멀티시그(다중서명) 트랜잭션이나 복잡한 조건을 가진 스마트 컨트랙트 형태의 잠금 스크립트를 사용할 때 활용된다. `3`으로 시작하는 주소로 보내면 내부적으로 스크립트 조건을 만족해야 출금이 가능하다.
...
2017년 세그윗(SegWit) 업그레이드와 함께 도입된 주소 형식이다. 수수료가 가장 저렴하고, 오류 감지 기능이 우수하다. 오늘날 새로 만드는 지갑 대부분이 기본 형식으로 사용한다. `bc1q`로 시작하므로 쉽게 구별된다.
...
레거시 주소(1로 시작)와 P2SH 주소(3으로 시작)는 대소문자를 구분한다. `1ABC`와 `1abc`는 다른 주소다. 반면 Bech32 주소(bc1)는 대소문자를 구분하지 않는다.
...
[txid.uk](https://txid.uk/)에서 주소를 검색하면 해당 주소와 연관된 모든 거래 내역과 현재 잔액을 확인할 수 있다.

## txid.uk로 주소 검증하기

주소가 유효한지, 잔액이 얼마인지 직접 확인하려면 txid.uk의 도구들을 활용하면 된다.

### 주소 유효성 검증

[tools.txid.uk](https://tools.txid.uk/)의 **주소 검증** 도구를 사용하면 주소 형식이 올바른지, 어떤 유형(P2PKH, P2SH, SegWit, Taproot)인지 즉시 확인할 수 있다. 전송 전 주소를 한 번 검증하는 습관을 들이면 실수를 방지할 수 있다.

### 주소 잔액·거래 내역 조회

[txid.uk](https://txid.uk/) 메인 탐색기에서 주소를 검색하면 잔액, 거래 내역, UTXO 목록을 한눈에 확인할 수 있다.

**실습 순서:**

1. [txid.uk](https://txid.uk/) 접속 후 검색창에 비트코인 주소 입력

![txid.uk 주소 검색 화면](/images/learn/address-validation-1.webp)

2. 주소 잔액과 전체 거래 내역 확인

![주소 잔액 및 거래 내역 화면](/images/learn/address-validation-2.webp)

3. UTXO(미사용 트랜잭션 출력) 상세 내역 조회

![UTXO 상세 보기 화면](/images/learn/address-validation-3.webp)

여러 주소의 잔액을 한꺼번에 추적하고 싶다면 [portfolio.txid.uk](https://portfolio.txid.uk/)를 활용할 수 있다. 멀티주소 잔액 추적 기능으로 보유 현황을 한 화면에서 관리할 수 있다.

주소를 검색한다고 해서 자산이 위험해지지는 않는다. 비트코인 주소와 거래 내역은 공개 정보이며, 조회 자체는 아무런 위험이 없다.

> **주의: 비트코인과 비트코인 캐시(BCH) 주소 혼동**
>
> 비트코인(BTC)과 비트코인 캐시(BCH)의 레거시 주소 형식은 동일하게 `1`로 시작한다. 비트코인 전송 시 비트코인 캐시 주소로 잘못 보내거나, 그 반대의 실수가 발생할 수 있다. 전송 전 지갑 소프트웨어가 올바른 네트워크를 선택하고 있는지 반드시 확인해야 한다. 잘못된 네트워크로 전송된 비트코인은 복구가 매우 어렵거나 불가능하다.
