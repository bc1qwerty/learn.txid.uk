...
반감기 분석에서 자주 등장하는 **재고 대 유량(Stock-to-Flow, S2F) 모델**은 현존하는 재고량을 연간 신규 생산량으로 나눈 비율이다.
...
이 모델을 처음 대중화한 PlanB(익명의 네덜란드 기관 투자자)는 S2F 비율과 시장 가치가 강한 상관관계를 가진다고 주장했다. 비록 모델이 항상 정확히 들어맞지는 않지만, 공급 희소성과 가치 사이의 관계를 설명하는 직관적 프레임워크로 광범위하게 활용된다.
...
"채굴자들이 반감기 이후 채산성이 맞지 않아 채굴을 그만두면 비트코인이 붕괴한다"는 우려도 있다. 하지만 비트코인의 난이도 조절 메커니즘이 이를 자동으로 보정한다. 채굴자가 줄면 난이도가 내려가고, 남은 채굴자들이 더 쉽게 블록을 찾아 여전히 이익을 낼 수 있다. 네트워크는 자기 교정적으로 설계되어 있다.
