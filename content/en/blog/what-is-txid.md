...
1. All of the transaction's data (inputs, outputs, amounts, locking scripts, etc.) is serialized into a byte array.
2. The **SHA-256 hash function is applied twice** to this byte array (double SHA-256).
3. The **byte order of the resulting hash is reversed**.
...
You can paste these TXIDs into the search bar at [txid.uk](https://txid.uk/) to view the full details of each transaction.
...
1. Go to [txid.uk](https://txid.uk/) and paste the 64-character TXID into the search bar.
2. On the transaction detail page, review the input/output structure, fee, confirmation count, and block height.
3. On [viz.txid.uk](https://viz.txid.uk/), visualize the transaction's input/output relationships as a network graph -- try entering the Bitcoin Pizza transaction TXID to trace the flow of 10,000 BTC yourself.
4. On [tx.txid.uk](https://tx.txid.uk/), decode the raw transaction hex to see the structure from which the TXID was derived.
...
You can search for a TXID on [txid.uk](https://txid.uk/) to check its current confirmation count in real time. If it is unconfirmed, it will be displayed as "Unconfirmed."
